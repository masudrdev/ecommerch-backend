...
export default function VendorEarningsPage() {
  ...
  const fetchData = async (page = 1, firstLoad = false, filters = {}) => {
    try {
      ...
      const res = await financeService.getVendorEarnings({
        page,
        limit: 20,
  ...
  if (pageLoading) return <div className="text-slate-400">Loading 
  ...</div>;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">View completed vendor earnings after platform commission.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <Card title="Vendor Earnings" value={money(summary.totalVendorEarning)} />
        <Card title="Platform Commission" value={money(summary.totalCommission)} />
        <Card title="Completed Orders" value={summary.totalOrders} />
      </div>

      <div className="rounded-2xl border border-slate-700 bg-slate-800 p-4 md:p-5">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-white">Total Earnings History</h2>
        ...
        <div className="w-full overflow-x-auto">
          <table className="min-w-[1300px] w-full text-left text-sm">
            <thead className="border-b border-slate-700 text-slate-300"><tr>
              <th className="py-3">Order ID</th><th>Customer</th><th>Vendor Username</th><th>Product</th><th>Sale Amount</th><th>Commission</th><th>Vendor Earning</th><th>Payment</th><th>Status</th><th>Date</th>
            </tr></thead>
            <tbody>
              {items.map((item, index) => <tr key={`${item.orderId}-${item.product}-${index}`} className="border-b border-slate-700">
                <td className="py-3 text-white">{item.orderId}</td><td>{item.customer}</td><td>{item.vendorUsername}</td><td>{item.product}</td><td>{money(item.salePrice)}</td><td className="text-blue-400">{money(item.commission)}</td><td className="font-semibold text-green-400">{money(item.vendorEarning)}</td><td><StatusBadge status={item.paymentStatus} /></td><td><StatusBadge status={item.orderStatus} /></td><td className="text-slate-400">{new Date(item.date).toLocaleDateString()}</td>
              </tr>)}
              {!items.length && <tr><td colSpan="10" className="py-8 text-center text-slate-400">No completed earning records found.</td></tr>}
            </tbody>
          </table>
        </div>
